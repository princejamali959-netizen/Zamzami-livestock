...
const T = {
  en: {
    ...
    manageAccessTitle: "Manage access", manageAccessDesc: "Add people who can sign in. Choose whether each person can only view the ledger, or has full owner access.",
  ...
  ar: {
    ...
    manageAccessTitle: "إدارة الوصول", manageAccessDesc: "أضف الأشخاص الذين يمكنهم تسجيل الدخول. اختر إن كان بإمكان كل شخص مشاهدة السجل فقط أو لديه صلاحية المالك الكاملة.",
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [s, tr, w, l, ul, us] = await Promise.allSettled([
        ...
        if (us.status === "fulfilled" && us.value) setUsers(JSON.parse(us.value.value));
  ...
  const saveUsers = (v) => persist("users", v, setUsers);
  ...
  return (
    <div dir={dir} className="min-h-screen" style={{ background: "#F3EEE1", fontFamily: lang === "ar" ? "'Cairo','Noto Kufi Arabic',sans-serif" : "'Inter',sans-serif", color: "#24301F" }}>
      ...
      <main className="max-w-6xl mx-auto px-5 py-6">
        ...
        {activeTab === "settings" && (
          <SettingsTab t={t} isOwner={isOwner} settings={settings} saveSettings={saveSettings} transactions={transactions} workers={workers} livestock={livestock} users={users} saveUsers={saveUsers} />
        )}
      </main>
...
function SettingsTab({ t, isOwner, settings, saveSettings, transactions, workers, livestock, users, saveUsers }) {
  const [newPin, setNewPin] = useState("");
  const [newUser, setNewUser] = useState({ email: "", password: "", role: "viewer" });

  function addUser() {
    const email = newUser.email.trim().toLowerCase();
    if (!email || !newUser.password) return;
    const withoutDupe = users.filter(u => u.email.toLowerCase() !== email);
    saveUsers([...withoutDupe, { id: uid(), email, password: newUser.password, role: newUser.role }]);
      ...
      <div className="card p-4">
        ...
        <div className="grid grid-cols-2 gap-2 items-end mb-3">
          <div className="col-span-2">
            <label className="text-xs text-[#8A7F6B] block mb-1">{t("emailPh")}</label>
            <input type="email" value={newUser.email} onChange={e => setNewUser({ ...newUser, email: e.target.value })} className="w-full px-2 py-2 rounded-lg border border-[#E4DBC6] bg-white text-sm" />
          </div>
          <div>
            <label className="text-xs text-[#8A7F6B] block mb-1">{t("passwordPh")}</label>
            <input type="password" value={newUser.password} onChange={e => setNewUser({ ...newUser, password: e.target.value })} className="w-full px-2 py-2 rounded-lg border border-[#E4DBC6] bg-white text-sm" />
          </div>
          <div>
            <label className="text-xs text-[#8A7F6B] block mb-1">{t("userRoleLabel")}</label>
            <select value={newUser.role} onChange={e => setNewUser({ ...newUser, role: e.target.value })} className="w-full px-2 py-2 rounded-lg border border-[#E4DBC6] bg-white text-sm">
              <option value="viewer">{t("viewerRoleOpt")}</option>
              <option value="owner">{t("ownerRoleOpt")}</option>
            </select>
          </div>
        </div>
        <button onClick={addUser} className="px-4 py-2 rounded-lg bg-[#4F6B3D] text-white text-sm font-medium flex items-center gap-1.5 hover:bg-[#3f5731]"><Plus size={15} />{t("addUserBtn")}</button>
        <div className="mt-4 space-y-1.5">
          {users.length === 0 ? <p className="text-sm text-[#8A7F6B]">{t("noUsersYet")}</p> : users.map(u => (
            <div key={u.id} className="flex items-center justify-between text-sm py-1.5 border-b border-[#EDE6D2] last:border-0">
              ...
              <button onClick={() => removeUser(u.id)} className="text-xs text-[#A63D2F] font-medium">{t("removeUserBtn")}</button>
            </div>
          ))}
        </div>
      </div>
